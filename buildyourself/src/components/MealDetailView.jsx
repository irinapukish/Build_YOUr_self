...
import { convertUserInfoDtoToUserInfo } from "../helpers/modelConverterCommonHelper";
import { getMealCalority } from "../helpers/weeklyMenuPlanBuilder";
import meals from "../helpers/database.js";

const MealDetailView = () => {
  const { user } = useAuth();
  const [requiredCaloriesPerMeal, setRequiredCaloriesPerMeal] = useState(null);
  ...
  async function getUserInfo() {
    try {
      ...
      let userInfo = convertUserInfoDtoToUserInfo(response.data);
      ...
      setRequiredCaloriesPerMeal(calories / 3);
    } catch (error) {
            ...
            <div style={styles.header}>
              ...
              <div style={styles.details}>
                ...
                <div style={styles.preparationTime}>
                  ...
                  <div>
                    Czas przygotowania:{" "}
                    <strong>{recipeData.preparationTime} min</strong>
                  </div>
                  <div style={styles.timeIcon}>⏱️</div>
                  <div>
                    Całkowity czas przygotowania:{" "}
                    <strong>{recipeData.totalTime} min</strong>
                  </div>
                </div>
                {/* Macronutrients */}
              ...
              <div style={styles.preparationSection}>
                ...
                <ol style={styles.preparationList}>
                  {recipeData.preparation_method.map((step, index) => (
                    <li key={index} style={styles.preparationStep}>
                      {step}
                    </li>
                  ))}
                </ol>
